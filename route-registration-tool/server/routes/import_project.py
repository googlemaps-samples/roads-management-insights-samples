...
def _validate_session_id(session_id: str) -> str:
    try:
        ...
        raise ValueError("Invalid session_id (expected UUID).")
...
async def import_project_from_zip(
    ...
    try:
        route_columns_info = inspect(engine).get_columns("routes")
        route_columns = {col["name"] for col in route_columns_info}
        route_boolean_columns = {
            col["name"]
            for col in route_columns_info
            if isinstance(col["type"], Boolean)
        }
        normalized_session_id = _validate_session_id(session_id)
        # Keep FK integrity for projects.session_id.
        ...
        # These are DB-managed or can cause PK collisions when importing snapshots.
        ignored_route_fields = {"id", "created_at", "updated_at", "deleted_at"}

        replace_project_id = None
        existing_name_row = None
        with engine.connect() as _conn:
            existing_name_row = _conn.execute(
                text(
                    """
        ...
        if existing_name_row:
            replace_project_id = existing_name_row[0]
            if not force_replace:
                raise ValueError(
                    f"PROJECT_EXISTS::Project '{existing_name_row[1]}' already exists. "
                    "Confirm replace to delete the existing project and continue import."
                )
        ...
        if not ENABLE_MULTITENANT and google_cloud_project_id:
            with engine.connect() as _conn:
                existing_row = _conn.execute(
                    text(
                        "SELECT id, project_name FROM projects WHERE google_cloud_project_id = :gcp AND deleted_at IS NULL"
                    ),
                    {"gcp": google_cloud_project_id},
                ).fetchone()
                if existing_row and existing_row[0] != replace_project_id:
                    raise ValueError(
                        f"You're trying to upload a project with a Google Cloud Project ID that's already in use by '{existing_row[1]}'."
                    )
        with engine.begin() as conn:
            if force_replace and replace_project_id is not None:
                conn.execute(
            ...
            if existing_project:
                project_id = conn.execute(
                    text("""
                        INSERT INTO projects (project_uuid, project_name, jurisdiction_boundary_geojson, google_cloud_project_id,
                            google_cloud_project_number, subscription_id, dataset_name, viewstate, map_snapshot, session_id)
                ...
                conn.execute(text(""" INSERT INTO projects (id, project_uuid, project_name, jurisdiction_boundary_geojson, google_cloud_project_id, google_cloud_project_number, subscription_id, dataset_name, viewstate, map_snapshot, session_id) VALUES (:id, :project_uuid, :project_name, :jurisdiction_boundary_geojson, :google_cloud_project_id, :google_cloud_project_number, :subscription_id, :dataset_name, :viewstate, :map_snapshot, :session_id) """), {
                    "id": project_id,
                    "project_uuid": project_uuid_val,
                    "project_name": project_name,
                    "jurisdiction_boundary_geojson": jurisdiction_boundary_geojson,
            ...
            for route in routes_list:
                ...
                filtered_route = {
                    ...
                    if key in route_columns and key not in ignored_route_fields
                }
                for key in route_boolean_columns:
                    if key in filtered_route:
                        filtered_route[key] = _coerce_bool(filtered_route[key])

                if "uuid" not in filtered_route:
                    skipped += 1
                    continue

                # Build dynamic SQL for route insert
                cols = ", ".join(filtered_route.keys())
                vals = ", ".join([f":{k}" for k in filtered_route.keys()])
        ...
        raise ValueError(f"Database error occurred: {error_msg}")
...
async def import_project(
    ...
    try:
        ...
        result = await import_project_from_zip(
            zip_bytes,
